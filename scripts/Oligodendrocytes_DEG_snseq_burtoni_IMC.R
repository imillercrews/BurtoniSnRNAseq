...
#### clustering for oligodendrocytes ####
burtoni.snseq.combined.sct.oligodendrocytes = burtoni.snseq.combined.sct

#set idents
Idents(object = burtoni.snseq.combined.sct.oligodendrocytes) <- "sctypemarkers.hypo"

#subset to oligodendrocytes
burtoni.snseq.combined.sct.oligodendrocytes = subset(burtoni.snseq.combined.sct.oligodendrocytes,
                                            idents = c("C7-4: Oligo+Precursor"))

#remove large file
# rm(burtoni.snseq.combined.sct)

# need to set to integrated for clustering
DefaultAssay(burtoni.snseq.combined.sct.oligodendrocytes) = 'integrated'

#check data loaded correctly
## run PCA, UMAP, and cluster 
#use 0.2 resolution
burtoni.snseq.combined.sct.oligodendrocytes = burtoni.snseq.combined.sct.oligodendrocytes %>% 
  RunPCA() %>%
...
Idents(object = burtoni.snseq.combined.sct.oligodendrocytes) <- "integrated_snn_res.0.2"

DimPlot(burtoni.snseq.combined.sct.oligodendrocytes,
        group.by='integrated_snn_res.0.2',
        label=TRUE) +
  ggtitle('oligodendrocytes') +
  NoLegend() +
  theme_classic()
ggsave('oligodendrocytes/UMAP oligodendrocytes clusters.png',
       height = 5.5,
       width = 5.5)

# #check per genotype
# #use all oligodendrocytes
DimPlot(burtoni.snseq.combined.sct.oligodendrocytes,
        group.by='Genotype.id') +
  ggtitle('oligodendrocytes')+
  theme_classic()
ggsave('oligodendrocytes/UMAP oligodendrocytes genotypes.png',
       height = 5.5,
       width = 5.5)

## create count across genotype per cluster
burtoni.snseq.combined.sct.oligodendrocytes@meta.data %>%
  as.data.frame() %>%
  ...
  geom_jitter(height = 0,
              width = 0.1) +
  theme_classic()
ggsave('oligodendrocytes/Genotype per clusters count.png',
       height = 5.5,
       width = 5.5)
...
# add oligodendrocytes metadata
UMAP.orig = UMAP.orig %>% 
  full_join(burtoni.snseq.combined.sct.oligodendrocytes@meta.data %>%
                        as.data.frame() %>% 
                        rownames_to_column('Cell.id') %>% 
                        select(c(Cell.id,
                                 integrated_snn_res.0.2))) 
...
UMAP.orig %>% 
  ...
  geom_point(aes(color = integrated_snn_res.0.2)) +
  theme_classic() +
  labs(color = 'oligodendrocytes clusters')
ggsave('oligodendrocytes/UMAP projection oligodendrocytes clusters.png',
       height = 10,
       width = 12.5)
...
burtoni.snseq.combined.sct.oligodendrocytes.clustree <- Seurat::FindClusters(object = burtoni.snseq.combined.sct.oligodendrocytes,
                                                                               resolution = resolution.range.reduced.2)
#check data
# head(burtoni.snseq.combined.sct.oligodendrocytes.recluster.clustree[[]])
...
#                         '#b3b3b3')
#clustree
clustree(burtoni.snseq.combined.sct.oligodendrocytes.clustree,
...
ggsave('oligodendrocytes/Clustree oligodendrocytes.png',
       height = 5.5,
       width = 5.5)

# remove large file
rm(burtoni.snseq.combined.sct.oligodendrocytes.clustree)
...
burtoni.snseq.combined.sct.oligodendrocytes.variable.prep <- FindVariableFeatures(burtoni.snseq.combined.sct.oligodendrocytes, 
                                                                                              assay = 'integrated',
                                                                                              selection.method = "vst", 
                                                                                              nfeatures = 1500, 
                                                                                              verbose = F)
# identify top 1500 variable genes
oligodendrocytes.topgenes.prep <- head(VariableFeatures(burtoni.snseq.combined.sct.oligodendrocytes.variable.prep,
                                                               assay = 'integrated'), 
                                              1500)

# create dummy
burtoni.snseq.combined.sct.oligodendrocytes.expression.sct.prep = full_join(full_join(burtoni.snseq.combined.sct.oligodendrocytes@reductions$umap@cell.embeddings %>% 
                                                                                                   as.data.frame() %>% 
                                                                                                   rownames_to_column("Cell.id"),
                                                                                burtoni.snseq.combined.sct.oligodendrocytes@meta.data %>%
                                                                                                   rownames_to_column("Cell.id")),
                                                                      burtoni.snseq.combined.sct.oligodendrocytes@assays$SCT@data %>% 
                                                                                         as.data.frame()  %>% 
                                                                                         t() %>% as.data.frame() %>% 
                                                                                         rownames_to_column('Cell.id'))

## create vector of factor
oligodendrocytes.DomvsSub.vector.list.sct.prep = burtoni.snseq.combined.sct.oligodendrocytes.expression.sct.prep %>% 
...
oligodendrocytes.DomvsSub.vector.count.sct.prep = GetAssayData(burtoni.snseq.combined.sct.oligodendrocytes,
                                                                assay = 'SCT') %>% 
  as_tibble(rownames = NA) %>% 
  rownames_to_column('gene') %>% 
  dplyr::select(c(gene,
                  burtoni.snseq.combined.sct.oligodendrocytes.expression.sct.prep %>% 
                    pull(Cell.id))) %>% 
  filter(gene %in% oligodendrocytes.topgenes.prep) %>% 
  column_to_rownames('gene') %>% 
  as.matrix() %>% 
  pmax(0)


## run with oligodendrocytes clusters
#need list with L with  count and condt
# add in cell cluster with condt.2
## create vector of factor
oligodendrocytes.DomvsSub.vector.list.cluster.sct.prep = burtoni.snseq.combined.sct.oligodendrocytes.expression.sct.prep %>% 
  mutate(oligodendrocytes.cluster = integrated_snn_res.0.2 %>% 
           as.factor()) %>% 
  pull(oligodendrocytes.cluster) %>% 
  droplevels


# add in genotype
## create vector of factor
oligodendrocytes.DomvsSub.vector.list.genotype.sct.prep = burtoni.snseq.combined.sct.oligodendrocytes.expression.sct.prep %>% 
...
#create list
oligodendrocytes.vector.limma.sct.prep = list(count = oligodendrocytes.DomvsSub.vector.count.sct.prep,
                                               condt.2 = oligodendrocytes.DomvsSub.vector.list.cluster.sct.prep,
                                               genotype = oligodendrocytes.DomvsSub.vector.list.genotype.sct.prep)
# add genotype.ID?
...
run_limmatrend_complex_genotype.sct <- function(L) {
  ...
  # 
  # # Open pdf file
  # pdf(file= "./oligodendrocytes/limmatrend/limmatrend.MDS.pdf" )
  # # create a 2X1 grid
  # par( mfrow= c(2,1) )
  # limma::plotMDS(dge, 
...
###run function  
oligodendrocytes.genotype.vector.limma.results.sct = run_limmatrend_complex_genotype.sct(oligodendrocytes.vector.limma.sct.prep)

# save results to dataframe
oligodendrocytes.genotype.vector.limma.results.df.sct.treat.prep = full_join(oligodendrocytes.genotype.vector.limma.results.sct$tt0 %>% 
                                                                                rename_with(~paste0(.,"_DvsS_0")) %>% 
                                                                                rownames_to_column("Gene"),
                                                                              oligodendrocytes.genotype.vector.limma.results.sct$tt1 %>% 
                                                                                rename_with(~paste0(.,"_DvsS_1")) %>% 
                                                                                rownames_to_column("Gene")) %>% 
  full_join(oligodendrocytes.genotype.vector.limma.results.sct$tt2 %>% 
              rename_with(~paste0(.,"_DvsS_2")) %>% 
              rownames_to_column("Gene")) %>% 
  full_join(oligodendrocytes.genotype.vector.limma.results.sct$tt3 %>% 
              rename_with(~paste0(.,"_DvsS_3")) %>% 
              rownames_to_column("Gene")) %>% 
  full_join(oligodendrocytes.genotype.vector.limma.results.sct$tt4 %>% 
              rename_with(~paste0(.,"_DvsS_4")) %>% 
              rownames_to_column("Gene")) %>% 
  full_join(oligodendrocytes.genotype.vector.limma.results.sct$ttDS %>% 
...
oligodendrocytes.genotype.vector.limma.results.df.sct.treat.prep = oligodendrocytes.genotype.vector.limma.results.df.sct.treat.prep %>% 
...
for (i in limma.genotpye.vector) {
  # graph volcano plot
  oligodendrocytes.genotype.vector.limma.results.df.sct.treat.prep %>% 
  ...
  ggsave(paste0('oligodendrocytes/limmatrend/limma.oligodendrocytes.volcano.', i, '.png'),
         width = 5,
         height = 5)
}
...
#### save results ####
# save to csv
write_csv(oligodendrocytes.genotype.vector.limma.results.df.sct.treat.prep,
          file = 'oligodendrocytes/limmatrend/oligodendrocytes.limmatrend.results.csv')

##### poster heatmap ####
library(pheatmap)

# create matrix
oligodendrocytes.deg.matrix = oligodendrocytes.genotype.vector.limma.results.df.sct.treat.prep %>%
  filter(Keep == "Keep") %>% 
  column_to_rownames('Gene') %>% 
  select(c(All,
           '0',
...
oligodendrocytes.deg.matrix[!is.finite(oligodendrocytes.deg.matrix)] <- oligodendrocytes.deg.matrix %>% 
  as.data.frame() %>% 
...
oligodendrocytes.deg.matrix %>% 
  pheatmap(cluster_rows = F,
           ...
           filename = "oligodendrocytes/limmatrend/Heatmap DEGs across clusters.pdf",
           width = 12,
           height = 5
  )
#png
oligodendrocytes.deg.matrix %>% 
  pheatmap(cluster_rows = F,
           ...
           fontsize = 15,
           breaks = seq(-4, 4, length.out = 100),
           filename = "oligodendrocytes/limmatrend/Heatmap DEGs across clusters.png",
           width = 12,
           height = 5
  )

# create heatmap
oligodendrocytes.deg.phtmap <- pheatmap::pheatmap(oligodendrocytes.deg.matrix,
                                            cluster_rows = F,
                                            cluster_cols = T,
                                            scale = 'none',
                                            silent = T)
...
# get order list
oligodendrocytes.deg.order.list = c(oligodendrocytes.deg.phtmap$tree_col$order)
# create label list
oligodendrocytes.deg.order = data.frame(label = oligodendrocytes.deg.phtmap$tree_col$labels) %>% 
  rownames_to_column('position')
# combine labels and position
oligodendrocytes.deg.order = oligodendrocytes.deg.order[match(oligodendrocytes.deg.order.list,
                                                  oligodendrocytes.deg.order$position),] %>% 
  mutate(oligodendrocytes.deg.order.list = 1:length(oligodendrocytes.deg.order.list)) %>% 
  mutate(position = as.numeric(position))
# create new order list
oligodendrocytes.deg.order.list.new = oligodendrocytes.deg.order %>%
  arrange(position) %>% 
  pull(label)

# move Dom bias genes to end
oligodendrocytes.deg.col_dend <- oligodendrocytes.deg.phtmap[[2]]
oligodendrocytes.deg.col_dend <- dendextend::rotate(oligodendrocytes.deg.col_dend, 
                                              order = oligodendrocytes.deg.order.list.new)
# graph pheat map
# pdf
pheatmap(oligodendrocytes.deg.matrix, 
         cluster_cols=as.hclust(oligodendrocytes.deg.col_dend),
         cluster_rows = F,
         gaps_row = 1,
         scale = 'none',
         border_color = 'black',
         ...
         filename = "oligodendrocytes/limmatrend/Heatmap DEGs across clusters reorder.pdf",
         width = 12,
         height = 5
)
...
pheatmap(oligodendrocytes.deg.matrix, 
         ...
         fontsize = 15,
         breaks = seq(-2, 2, length.out = 100),
         filename = "oligodendrocytes/limmatrend/Heatmap DEGs across clusters reorder.png",
         width = 12,
         height = 5
)
