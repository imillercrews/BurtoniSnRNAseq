...
#### clustering for astrocytes ####
burtoni.snseq.combined.sct.astrocytes = burtoni.snseq.combined.sct

#set idents
Idents(object = burtoni.snseq.combined.sct.astrocytes) <- "sctypemarkers.hypo"

#subset to astrocytes
burtoni.snseq.combined.sct.astrocytes = subset(burtoni.snseq.combined.sct.astrocytes,
                                            idents = c("C7-3: Astro-Ependymal"))

#remove large file
# rm(burtoni.snseq.combined.sct)

# need to set to integrated for clustering
DefaultAssay(burtoni.snseq.combined.sct.astrocytes) = 'integrated'

#check data loaded correctly
## run PCA, UMAP, and cluster 
#use 0.8 resolution
burtoni.snseq.combined.sct.astrocytes = burtoni.snseq.combined.sct.astrocytes %>% 
  RunPCA() %>%
...
Idents(object = burtoni.snseq.combined.sct.astrocytes) <- "integrated_snn_res.0.2"

DimPlot(burtoni.snseq.combined.sct.astrocytes,
        group.by='integrated_snn_res.0.2',
        label=TRUE) +
  ggtitle('astrocytes') +
  NoLegend() +
  theme_classic()
ggsave('astrocytes/UMAP astrocytes clusters.png',
       height = 5.5,
       width = 5.5)
...
DimPlot(burtoni.snseq.combined.sct.astrocytes,
  ...
  ggtitle('astrocytes')+
  theme_classic()
ggsave('astrocytes/UMAP astrocytes genotypes.png',
       height = 5.5,
       width = 5.5)

## create count across genotype per cluster
burtoni.snseq.combined.sct.astrocytes@meta.data %>%
  as.data.frame() %>%
  ...
  geom_jitter(height = 0,
              width = 0.1) +
  theme_classic()
ggsave('astrocytes/Genotype per clusters count.png',
       height = 5.5,
       width = 5.5)
...
# add astrocytes metadata
UMAP.orig = UMAP.orig %>% 
  full_join(burtoni.snseq.combined.sct.astrocytes@meta.data %>%
              as.data.frame() %>% 
              rownames_to_column('Cell.id') %>% 
              select(c(Cell.id,
                       integrated_snn_res.0.2))) 
...
UMAP.orig %>% 
  ...
  theme_classic() +
  labs(color = 'astrocytes clusters')
ggsave('astrocytes/UMAP projection astrocytes clusters.png',
       height = 10,
       width = 12.5)
...
burtoni.snseq.combined.sct.astrocytes.clustree <- Seurat::FindClusters(object = burtoni.snseq.combined.sct.astrocytes,
                                                                               resolution = resolution.range.reduced.2)
#check data
# head(burtoni.snseq.combined.sct.astrocytes.recluster.clustree[[]])
...
#                         '#b3b3b3')
#clustree
clustree(burtoni.snseq.combined.sct.astrocytes.clustree,
...
ggsave('astrocytes/Clustree astrocytes.png',
       height = 5.5,
       width = 5.5)

# remove large file
rm(burtoni.snseq.combined.sct.astrocytes.clustree)
...
burtoni.snseq.combined.sct.astrocytes.variable.prep <- FindVariableFeatures(burtoni.snseq.combined.sct.astrocytes, 
                                                                                              assay = 'integrated',
                                                                                              selection.method = "vst", 
                                                                                              nfeatures = 1500, 
                                                                                              verbose = F)
# identify top 1500 variable genes
astrocytes.topgenes.prep <- head(VariableFeatures(burtoni.snseq.combined.sct.astrocytes.variable.prep,
                                                               assay = 'integrated'), 
                                              1500)

# create dummy
burtoni.snseq.combined.sct.astrocytes.expression.sct.prep = full_join(full_join(burtoni.snseq.combined.sct.astrocytes@reductions$umap@cell.embeddings %>% 
                                                                                                   as.data.frame() %>% 
                                                                                                   rownames_to_column("Cell.id"),
                                                                                burtoni.snseq.combined.sct.astrocytes@meta.data %>%
                                                                                                   rownames_to_column("Cell.id")),
                                                                      burtoni.snseq.combined.sct.astrocytes@assays$SCT@data %>% 
                                                                                         as.data.frame()  %>% 
                                                                                         t() %>% as.data.frame() %>% 
                                                                                         rownames_to_column('Cell.id'))

## create vector of factor
astrocytes.DomvsSub.vector.list.sct.prep = burtoni.snseq.combined.sct.astrocytes.expression.sct.prep %>% 
...
astrocytes.DomvsSub.vector.count.sct.prep = GetAssayData(burtoni.snseq.combined.sct.astrocytes,
                                                                assay = 'SCT') %>% 
  as_tibble(rownames = NA) %>% 
  rownames_to_column('gene') %>% 
  dplyr::select(c(gene,
                  burtoni.snseq.combined.sct.astrocytes.expression.sct.prep %>% 
                    pull(Cell.id))) %>% 
  filter(gene %in% astrocytes.topgenes.prep) %>% 
  column_to_rownames('gene') %>% 
  as.matrix() %>% 
  pmax(0)


## run with astrocytes clusters
#need list with L with  count and condt
# add in cell cluster with condt.2
## create vector of factor
astrocytes.DomvsSub.vector.list.cluster.sct.prep = burtoni.snseq.combined.sct.astrocytes.expression.sct.prep %>% 
  mutate(astrocytes.cluster = integrated_snn_res.0.2 %>% 
           as.factor()) %>% 
  pull(astrocytes.cluster) %>% 
  droplevels


# add in genotype
## create vector of factor
astrocytes.DomvsSub.vector.list.genotype.sct.prep = burtoni.snseq.combined.sct.astrocytes.expression.sct.prep %>% 
...
#create list
astrocytes.vector.limma.sct.prep = list(count = astrocytes.DomvsSub.vector.count.sct.prep,
                                               condt.2 = astrocytes.DomvsSub.vector.list.cluster.sct.prep,
                                               genotype = astrocytes.DomvsSub.vector.list.genotype.sct.prep)
# add genotype.ID?
...
run_limmatrend_complex_genotype.sct <- function(L) {
  ...
  timing <- system.time({
    ...
    contrasts <- makeContrasts(DvsS_0 = (treatDom.A.0 + treatDom.B.0 + treatDom.C.0)/3 - (treatSub.A.0 + treatSub.B.0 + treatSub.C.0)/3, 
                               ...
                               DvsS=(treatDom.A.0 + treatDom.B.0 + treatDom.C.0+treatDom.A.1 + treatDom.B.1 + treatDom.C.1+treatDom.A.2 + treatDom.B.2 + treatDom.C.2)/9-(treatSub.A.0 + treatSub.B.0 + treatSub.C.0+treatSub.A.1 + treatSub.B.1 + treatSub.C.1+treatSub.A.2 + treatSub.B.2 + treatSub.C.2)/9,
                               levels = design)
    dge <- DGEList(L$count, 
                   group = treat)
  ...
  # 
  # # Open pdf file
  # pdf(file= "./astrocytes/limmatrend/limmatrend.MDS.pdf" )
  # # create a 2X1 grid
  # par( mfrow= c(2,1) )
  # limma::plotMDS(dge, 
...
###run function  
astrocytes.genotype.vector.limma.results.sct = run_limmatrend_complex_genotype.sct(astrocytes.vector.limma.sct.prep)

# save results to dataframe
astrocytes.genotype.vector.limma.results.df.sct.treat.prep = full_join(astrocytes.genotype.vector.limma.results.sct$tt0 %>% 
                                                                                rename_with(~paste0(.,"_DvsS_0")) %>% 
                                                                                rownames_to_column("Gene"),
                                                                              astrocytes.genotype.vector.limma.results.sct$tt1 %>% 
                                                                                rename_with(~paste0(.,"_DvsS_1")) %>% 
                                                                                rownames_to_column("Gene")) %>% 
  full_join(astrocytes.genotype.vector.limma.results.sct$tt2 %>% 
              rename_with(~paste0(.,"_DvsS_2")) %>% 
              rownames_to_column("Gene")) %>% 
  full_join(astrocytes.genotype.vector.limma.results.sct$ttDS %>% 
...
astrocytes.genotype.vector.limma.results.df.sct.treat.prep = astrocytes.genotype.vector.limma.results.df.sct.treat.prep %>% 
...
for (i in limma.genotpye.vector) {
  # graph volcano plot
  astrocytes.genotype.vector.limma.results.df.sct.treat.prep %>% 
  ...
  ggsave(paste0('astrocytes/limmatrend/limma.astrocytes.volcano.', i, '.png'),
         width = 5,
         height = 5)
}
...
#### save results ####
# save to csv
write_csv(astrocytes.genotype.vector.limma.results.df.sct.treat.prep,
          file = 'astrocytes/limmatrend/astrocytes.limmatrend.results.csv')

##### poster heatmap ####
library(pheatmap)

# create matrix
astrocytes.deg.matrix = astrocytes.genotype.vector.limma.results.df.sct.treat.prep %>%
  filter(Keep == "Keep") %>% 
  column_to_rownames('Gene') %>% 
  select(c(All,
           '0',
...
astrocytes.deg.matrix %>% 
  pheatmap(cluster_rows = F,
           ...
           filename = "astrocytes/limmatrend/Heatmap DEGs across clusters.pdf",
           width = 12,
           height = 5
  )
#png
astrocytes.deg.matrix %>% 
  pheatmap(cluster_rows = F,
           ...
           fontsize = 15,
           breaks = seq(-4, 4, length.out = 100),
           filename = "astrocytes/limmatrend/Heatmap DEGs across clusters.png",
           width = 12,
           height = 5
  )
dev.off()
...
# create heatmap
astrocytes.deg.phtmap <- pheatmap::pheatmap(astrocytes.deg.matrix,
                                            cluster_rows = F,
                                            cluster_cols = T,
                                            scale = 'none',
                                            silent = T)
...
# get order list
astrocytes.deg.order.list = c(astrocytes.deg.phtmap$tree_col$order)
# create label list
astrocytes.deg.order = data.frame(label = astrocytes.deg.phtmap$tree_col$labels) %>% 
  rownames_to_column('position')
# combine labels and position
astrocytes.deg.order = astrocytes.deg.order[match(astrocytes.deg.order.list,
                                                  astrocytes.deg.order$position),] %>% 
  mutate(astrocytes.deg.order.list = 1:length(astrocytes.deg.order.list)) %>% 
  mutate(position = as.numeric(position))
# create new order list
astrocytes.deg.order.list.new = astrocytes.deg.order %>%
  arrange(position) %>% 
  pull(label)

# move Dom bias genes to end
astrocytes.deg.col_dend <- astrocytes.deg.phtmap[[2]]
astrocytes.deg.col_dend <- dendextend::rotate(astrocytes.deg.col_dend, 
                                              order = astrocytes.deg.order.list.new)
# graph pheat map
# pdf
pheatmap(astrocytes.deg.matrix, 
         cluster_cols=as.hclust(astrocytes.deg.col_dend),
         cluster_rows = F,
         gaps_row = 1,
         scale = 'none',
         border_color = 'black',
         ...
         filename = "astrocytes/limmatrend/Heatmap DEGs across clusters reorder.pdf",
         width = 12,
         height = 5
)
# png
pheatmap(astrocytes.deg.matrix, 
         ...
         fontsize = 15,
         breaks = seq(-2, 2, length.out = 100),
         filename = "astrocytes/limmatrend/Heatmap DEGs across clusters reorder.png",
         width = 12,
         height = 5
)
dev.off()  
