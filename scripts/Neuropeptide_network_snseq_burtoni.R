...
## get expression per cell
# use RNA counts
DefaultAssay(burtoni.snseq.combined.sct.all.neurons) <- "RNA"

# get gene by cell counts
burtoni.snseq.combined.sct.neurons.np.expression = FetchData(burtoni.snseq.combined.sct.all.neurons,
                                                             vars = neuropeptides.genes)

# rename ensemble genes
burtoni.snseq.combined.sct.neurons.np.expression = burtoni.snseq.combined.sct.neurons.np.expression %>% 
  dplyr::rename(
    ADM = ENSONIG00000036130,
...
# check counts
str(burtoni.snseq.combined.sct.neurons.np.expression)
# 'data.frame':	16194 obs. of  67 variables:

## create presence/absence matrix
# use dynamic threshold for broadly expressed genes
burtoni.snseq.combined.sct.neurons.np.expression.filter = apply(burtoni.snseq.combined.sct.neurons.np.expression, 2, function(x) {
  # across cells calculate percentage that express
  pct_expressing <- sum(x > 0) / length(x)
  
  # if more than 30% of cells express a gene take top 15%
  # if more than 10% of cells express a gene take 30% 
  # otherwise just look for expression presence 
  if (pct_expressing > 0.30) {
    cutoff <- quantile(x[x > 0], 0.85) 
  } else if (pct_expressing > 0.1) {
    cutoff <- quantile(x[x > 0], 0.70)
  } else {
    cutoff <- 0 
  }
...
# check cell names
identical(rownames(burtoni.snseq.combined.sct.neurons.np.expression.filter),
          rownames(burtoni.snseq.combined.sct.neurons.np.expression))

## get co-expression matrix
burtoni.snseq.combined.sct.neurons.np.expression.filter.co = t(burtoni.snseq.combined.sct.neurons.np.expression.filter) %*% 
  burtoni.snseq.combined.sct.neurons.np.expression.filter

# Get jaccard similarity
burtoni.snseq.combined.sct.neurons.np.expression.filter.similarity = as.matrix(proxy::simil(t(burtoni.snseq.combined.sct.neurons.np.expression.filter), 
                                                                                            method = "Jaccard"))
...
burtoni.snseq.combined.sct.neurons.np.expression.genotypes = burtoni.snseq.combined.sct.neurons.np.expression.filter  %>% 
  as.data.frame() %>% 
...
burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df = (burtoni.snseq.combined.sct.neurons.np.expression.filter) %>% 
  as.data.frame()
...
burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df.status.total.count %>% 
  mutate(prob = Total.neuropeptides*(Percent/100)) %>% 
  group_by(Genotype.id,
           Status) %>% 
  summarise(mean = sum(prob)) %>% 
  group_by(Status) %>% 
...
burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df.status.total.count %>%
  ...
  group_by(Genotype.id,
           Status) %>%
  summarize(median_val = median(Total.neuropeptides))
...
burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df.status.total.count %>% 
  ...
  scale_fill_manual(values = c("#4e499e",
                               "#60bb46"))+
  xlab('Neuropeptides per neuron')
ggsave('neuropeptides/comparison/social.status/Neuropeptides per neuron across status error paper.pdf',
       width = 6.5,
       height = 6.5,
       units = 'in',
       dpi = 720)
...
### create network across genotypes
# create dummy dataframe
...
vSizes.all.gene <- (scale01(apply(burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df %>% t(), 1, mean )) + 1.0) 
...
burtoni.snseq.neuropeptide.network.g <- delete_edges(burtoni.snseq.neuropeptide.network.g,
                                                     E(burtoni.snseq.neuropeptide.network.g)[which(E(burtoni.snseq.neuropeptide.network.g)$p.value.adj>0.15)])

# create names variable
V(burtoni.snseq.neuropeptide.network.g)$node_label <- names(V(burtoni.snseq.neuropeptide.network.g))
# create size variable
V(burtoni.snseq.neuropeptide.network.g)$node_size = vSizes.all.gene[V(burtoni.snseq.neuropeptide.network.g)$name]
...
# remove empty nodes
burtoni.snseq.neuropeptide.network.g = delete_vertices(
  burtoni.snseq.neuropeptide.network.g,
  V(burtoni.snseq.neuropeptide.network.g)[degree(burtoni.snseq.neuropeptide.network.g) == 0])
...
ggsave('neuropeptides/comparison/network/neuropeptides significant presence network paper.pdf',
       height = 6.5,
       width = 6.5,
       units = 'in',
       dpi = 720)
...
vSizes.all.gene.mat = (scale01(apply(burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df %>% t(), 1, mean )) + 1.0) 
# filter to matrix
vSizes.all.gene.value =
  vSizes.all.gene.mat  %>% 
  as.data.frame() %>% 
  arrange(desc(.)) %>% 
  rownames_to_column('gene') %>% 
  filter(gene %in% colnames(burtoni.snseq.neuropeptide.network.g.mat)) %>% 
  pull(gene)

# order by number of cells
burtoni.snseq.neuropeptide.network.g.mat.filter = burtoni.snseq.neuropeptide.network.g.mat[vSizes.all.gene.value, vSizes.all.gene.value]
# set diagonal to NA
diag(burtoni.snseq.neuropeptide.network.g.mat.filter) <- NA

## get significance matrix
burtoni.snseq.neuropeptide.network.sig.mat =
  burtoni.snseq.neuropeptide.network.sig %>% 
  rbind(burtoni.snseq.neuropeptide.network.sig %>% 
          dplyr::rename('To2' = 'From') %>% 
          dplyr::rename('From' = 'To') %>% 
          dplyr::rename('To' = 'To2')) %>% 
  distinct() %>% 
  mutate(p.value.adj = ifelse(p.value.adj < 0.15,
                              '*',
                              '')) %>% 
  pivot_wider(names_from = 'To',
              values_from = 'p.value.adj',
              values_fill = '') %>% 
  column_to_rownames('From') %>% 
  as.matrix()

# order by number of cells
burtoni.snseq.neuropeptide.network.sig.mat = burtoni.snseq.neuropeptide.network.sig.mat[vSizes.all.gene.value, vSizes.all.gene.value]

# set diagonal to NA
diag(burtoni.snseq.neuropeptide.network.sig.mat) <- ''


## create pheatmap
burtoni.snseq.neuropeptide.network.g.mat.filter %>% 
  pheatmap::pheatmap(na_col = 'grey',
                     cluster_cols = F,
                     cluster_rows = F,
                     ...
                     color = c('#60bb46',
                               ...
                               '#4e499e'),
                     angle_col = 45,
                     display_numbers = burtoni.snseq.neuropeptide.network.sig.mat,
                     fontsize_number = 10,    
                     number_color = "white",
                     legend = F,
                     filename = 'neuropeptides/comparison/network/neuropeptides significant presence network heatmap paper.pdf',
                     height = 13,
                     width = 13)
...
for (i in Genotype.id.list) {
  ...
  tmp <- delete_edges(tmp,
                      E(tmp)[which(E(tmp)$weight.scaled<0.1)])
  ...
  tmp.global = tmp %>%
    transitivity(type = 'average',
                 weights = E(tmp)$weight.scaled)
...
transitivty.df.wide = transitivty.df %>% 
  ...
  summarise(transitivity.avg = mean(transitivity,
                                    na.rm = T),
            weighted.degree.avg = mean(weighted.degree,
                                       na.rm = T)) %>%
  ungroup() %>% 
  pivot_wider(id_cols = 'vertice',
              names_from = 'orig.ident',
              values_from = c('transitivity.avg',
                              'weighted.degree.avg')) %>% 
  mutate(color = ifelse(vertice == 'global',
                        'global',
                        'local'),
         transitivity.avg_sub_burtoni_snseq = ifelse(is.na(transitivity.avg_sub_burtoni_snseq),
                                                     0,
                                                     transitivity.avg_sub_burtoni_snseq),
         transitivity.avg_dom_burtoni_snseq = ifelse(is.na(transitivity.avg_dom_burtoni_snseq),
                                                     0,
                                                     transitivity.avg_dom_burtoni_snseq),
         weighted.degree.avg.high = case_when(transitivity.avg_sub_burtoni_snseq > transitivity.avg_dom_burtoni_snseq ~ weighted.degree.avg_sub_burtoni_snseq,
                                              transitivity.avg_sub_burtoni_snseq < transitivity.avg_dom_burtoni_snseq ~ weighted.degree.avg_dom_burtoni_snseq,
                                              TRUE ~ max(weighted.degree.avg_dom_burtoni_snseq,
                                                         weighted.degree.avg_sub_burtoni_snseq,
                                                         na.rm = T)),
         weighted.degree.avg.diff = transitivity.avg_dom_burtoni_snseq - transitivity.avg_sub_burtoni_snseq
  ) 


## calculate significance
transitivity.sig = transitivty.df %>%
  group_by(vertice) %>%
  summarise(
    p.value = tryCatch(
      t.test(transitivity ~ orig.ident)$p.value,
      error = function(e) NA
    )
  ) %>%
  mutate(
...
# add to dataframe
transitivty.df.wide = transitivty.df.wide %>%
  left_join(transitivity.sig, 
            by = "vertice") %>% 
  mutate(label = case_when(p.adj < 0.05 ~ vertice,
                           vertice == 'global' ~ vertice,
                           abs(weighted.degree.avg.diff) > 0.5 ~ vertice,
                           TRUE ~ NA))
...
# paper
transitivty.df.wide %>% 
  filter(!(transitivity.avg_sub_burtoni_snseq == 0 & transitivity.avg_dom_burtoni_snseq == 0)) %>% 
  ggplot(aes(x = transitivity.avg_dom_burtoni_snseq,
             y = transitivity.avg_sub_burtoni_snseq,
             label = label)) +
  geom_abline(slope = 1,
              intercept = 0) +
  geom_point(aes(size = weighted.degree.avg.high,
                 fill = weighted.degree.avg.diff),
             shape = 21) +
  ...
  scale_fill_gradient2(midpoint = 0,
                       ...
                       low= '#60bb46',
                       mid = 'white') +
  labs(color = 'Transitivity',
       fill ='Strength difference',
       size = 'Strength max') +
...
ggsave('neuropeptides/comparison/network/Transitivity genotypes average paper.pdf',
       height = 6.5,
       width = 6.5,
       units = 'in',
       dpi = 720)
...
write_csv(neuropeptide.genes.glm,
          file = 'neuropeptides/comparison/social.status/neuropeptide.genes.glm.csv')

# neuropeptide.genes.glm = read.csv('neuropeptides/comparison/social.status/neuropeptide.genes.glm.csv')
...
neuropeptide.genes.glm.genes = neuropeptide.genes.glm %>% 
  ...
  filter(adj.pvalue.round <= 0.001) %>% 
  mutate(direction = ifelse(Estimate < 0,
                            "Dom.Bias",
                            "Sub.Bias")) %>% 
  arrange(adj.pvalue.round,
          direction) %>% 
...
neuropeptides.per.neuron.df.geno.count.percent.fig = neuropeptides.per.neuron.df.geno.count.percent %>% 
  ...
  mutate(label = ifelse(is.na(direction),
                        '',
                        neuropeptide),
         label = ifelse(Sub == 0 & Dom > 7.5,
                        neuropeptide,
                        label),
         label = ifelse(direction == 'Dom.Bias' & Dom < 7.5,
                        '',
                        label),
         direction = ifelse(is.na(direction),
                            'no bias',
                            direction),
         direction = ifelse(Sub ==0 & Dom > 5,
                            'Dom.Bias',
                            direction)) 
...
neuropeptides.per.neuron.df.geno.count.percent.fig %>% 
  ...
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        title =element_text(size=12)) 
ggsave('neuropeptides/comparison/social.status/Neuropeptides count comparison across social status paper glm paper.pdf', 
       height = 6.5,
       width = 6.5,
       units = 'in',
       dpi = 720)
...
neuropeptides.per.neuron.df.geno.count.percent.fig %>% 
  ...
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 12),
        title =element_text(size=12,)) 
ggsave('neuropeptides/comparison/social.status/Neuropeptides count comparison across social status paper glm.png', 
       height = 10,
       width = 10)
...
neuropeptides.per.neuron.df.geno.count.percent.fig %>% 
  ...
  theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 20),
        title =element_text(size=20, face='bold')) 
ggsave('neuropeptides/comparison/social.status/Neuropeptides count comparison across social status paper glm subset label.png', 
       height = 10,
       width = 10)
...
neuropeptides.per.neuron.df.geno.count.percent.fig %>% 
  ...
  theme(axis.text = element_text(size = 20),
        axis.title = element_text(size = 20),
        title =element_text(size=20, face='bold')) 
ggsave('neuropeptides/comparison/social.status/Neuropeptides count comparison across social status paper glm no label.png', 
       height = 10,
       width = 10)

#### testing covariance matrices ####
### filter network to only include neuropeptides with 10 cells per male
genes.keep = colSums(burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df) %>% 
  as.data.frame() %>% 
  rownames_to_column('gene') %>% 
  filter(. > 25) %>% 
  pull(gene)

### create individual co-expression matrix
network.list = list()
jaccard.list = list()
...
for (i in Genotype.id.list) {
  ...
  # filter presence/absence matrix
  tmp = burtoni.snseq.combined.sct.neurons.np.expression.filter.adj.df %>%
    filter(rownames(.) %in% tmp.names) %>% 
    dplyr::select(all_of(genes.keep))
  
  # create co-expression matrix
  tmp.mat = crossprod(as.matrix(tmp))
  
  # normalize by cell number
  tmp.mat = 100 * tmp.mat / nrow(tmp)
  
  network.list[[i]] = tmp.mat
  ...
  jaccard.list[[i]] = tmp.mat
  
}

# sample data 
sample.data = burtoni.snseq.combined.sct.all.neurons@meta.data %>% 
  distinct(orig.ident,
           Genotype.id)

## convert network to vector
# get upper tri
network.vecs = lapply(network.list, function(mat) {
  mat[upper.tri(mat, diag = FALSE)]
})

# combine together
network.df = do.call(rbind, network.vecs)

# add names
rownames(network.df) = names(network.list)

## convert jaccard to vector
# get upper tri
jaccard.vecs = lapply(jaccard.list, function(mat) {
  mat[upper.tri(mat, diag = FALSE)]
})

# combine together
jaccard.df = do.call(rbind, jaccard.vecs)

# add names
rownames(jaccard.df) = names(jaccard.list)
...
network.dist = dist(network.df)

# run permutation
adonis.res = adonis2(
  network.dist ~ orig.ident,
  data = sample.data,
  permutations = 9999)
...
network.corr = as.dist(1 - cor(t(network.df)))


# run permutation
adonis.res.cor = adonis2(
  network.corr ~ orig.ident,
  data = sample.data,
  permutations = 9999)
...
# check dispersion
disper.corr = betadisper(
  network.corr,
  sample.data$orig.ident)

permutest(disper.corr)
...
## distance
# create distance matrix
jaccard.dist = dist(jaccard.df)

# run permutation
jac.adonis.res = adonis2(
  jaccard.dist ~ orig.ident,
  data = sample.data,
  permutations = 9999)
...
# check dispersion
jac.disper = betadisper(
  jaccard.dist,
  sample.data$orig.ident)

permutest(jac.disper)
...
## correlation distance
# create correlation matrix
jaccard.corr = as.dist(1 - cor(t(jaccard.df)))


# run permutation
jac.adonis.res.cor = adonis2(
  jaccard.corr ~ orig.ident,
  data = sample.data,
  permutations = 9999)
...
# check dispersion
jac.disper.corr = betadisper(
  jaccard.corr,
  sample.data$orig.ident)

permutest(jac.disper.corr)

# graph dispersion
boxplot(jac.disper.corr)
plot(jac.disper.corr)

### correlation of edges
## network
network.sim = cor(t(network.df),
                  method = "pearson")

# create order of males
network.sim.df = expand.grid(
  male1 = rownames(network.sim),
  male2 = colnames(network.sim),
  stringsAsFactors = FALSE) %>%
  filter(male1 < male2)

# pairwise dataframe
network.sim.df$similarity = mapply(
  function(x,y) network.sim[x,y],
  network.sim.df$male1,
  network.sim.df$male2)

# add status
network.sim.df = network.sim.df %>%
  left_join(
    sample.data %>%
      dplyr::select(Genotype.id, orig.ident),
    by = c("male1" = "Genotype.id")) %>%
  rename(status1 = orig.ident) %>%
  left_join(
    sample.data %>%
      dplyr::select(Genotype.id, orig.ident),
    by = c("male2" = "Genotype.id")) %>%
  rename(status2 = orig.ident) %>%
  mutate(comparison = case_when(status1 == status2 & status1 == "dom_burtoni_snseq" ~ "Dom-Dom",
                                status1 == status2 & status1 == "sub_burtoni_snseq" ~ "Sub-Sub",
                                TRUE ~ "Dom-Sub"),
         comparison2 = ifelse(comparison == "Dom-Sub",
                              "Between",
                              "Within"))

# wilcox test
wilcox.test(similarity ~ comparison2,
  data = network.sim.df)
...
pairwise.wilcox.test(
  network.sim.df$similarity,
  network.sim.df$comparison,
  p.adjust.method = "fdr"
)
...
## jaccard
jaccard.sim = cor(t(jaccard.df),
                  method = "pearson")

# create order of males
jaccard.sim.df = expand.grid(
  male1 = rownames(jaccard.sim),
  male2 = colnames(jaccard.sim),
  stringsAsFactors = FALSE) %>%
  filter(male1 < male2)

# pairwise dataframe
jaccard.sim.df$similarity = mapply(
  function(x,y) jaccard.sim[x,y],
  jaccard.sim.df$male1,
  jaccard.sim.df$male2)

# add status
jaccard.sim.df = jaccard.sim.df %>%
  left_join(
    sample.data %>%
      dplyr::select(Genotype.id, orig.ident),
    by = c("male1" = "Genotype.id")) %>%
...
# pairwise wilcox
pairwise.wilcox.test(
  jaccard.sim.df$similarity,
  jaccard.sim.df$comparison,
  p.adjust.method = "fdr"
)
...
# 
# 
# #### OLD: testing covariance matrices ####
...
# 
# #### OLD: clustering coefficient/ transitivity ####
# ## global
# #dom = 0.5159744
# burtoni.snseq.combined.sct.neurons.np.expression.filter.male.dom.g %>% 
#   transitivity(type = 'global')
...
# 
# 
# #### OLD: clustering coefficient/transitivity positive vs negative correlation ####
# ### dom
# ## create graph object
...
# #### OLD: neuropeptides across social status ####
# ### extract neuropeptide expression and orig.idents from neurons
# ## create expression dataframe of neuropeptides
# burtoni.snseq.combined.sct.neurons.np.expression = burtoni.snseq.combined.sct.all.neurons@assays$SCT@data %>% 
#   as.data.frame() %>% 
...
#                  nudge_y = p$data$y * .1)
# 
# ggsave('neuropeptides/comparison/network/neuropeptides significant presence network paper.png',
#        height = 6,
#        width = 6)
# 
# ### graph heatmap
...
#            angle_col = 45,
#            legend = F,
#            filename = 'neuropeptides/comparison/network/neuropeptides significant presence network heatmap paper.png',
#            height = 6.5,
#            width = 6.5)
# 
# burtoni.snseq.neuropeptide.network.g.mat %>% 
...
#   xlab('Dominant transitivity score')+
#   ylab('Subordinate transitivity score')
# ggsave('neuropeptides/comparison/network/Transitivity genotypes average paper.png',
#        height = 5.25,
#        width = 5.25)
# 
# 

# #### OLD: neuropeptide count statistics ####
# ### compare number of neuropeptide expressing cells across samples
# ### GLM binomial
# library(multcomp)
# library(emmeans)
