...
# create modulescore
burtoni.snseq.combined.sct.neurons = AddModuleScore(burtoni.snseq.combined.sct.neurons,
                                                                         features = list(IEG.list),
                                                                         name = "IEG_score")
...
burtoni.snseq.combined.sct.all.avp.neurons.recluster = AddMetaData(burtoni.snseq.combined.sct.all.avp.neurons.recluster,
                                                                   burtoni.snseq.combined.sct.all.avp.neurons.recluster@meta.data$seurat_clusters,
                                                         col.name = 'avp_clusters')

#set idents
Idents(object = burtoni.snseq.combined.sct.all.avp.neurons.recluster) <- "avp_clusters"
...
#         label=TRUE) +
#   ggtitle('Neurons AVP')
# ggsave('neuropeptides/avp.oxt/avp/UMAP.orig.ident.avp.neurons.all.png',
#        width = 10,
#        height = 10)
# # clusters
# DimPlot(burtoni.snseq.combined.sct.all.avp.neurons.recluster,
...
# # neuron clusters
# DimPlot(burtoni.snseq.combined.sct.all.avp.neurons.recluster,
#         group.by='neuron_clusters',
#         label=TRUE) +
#   ggtitle('Neurons AVP: neuron clusters')
# ggsave('neuropeptides/avp.oxt/avp/UMAP.neuron.clusters.avp.neurons.all.png',
#        width = 10,
#        height = 10)
# ## clusters
# # avp expression
...
# get around legacy code by renaming object
...
avp.avg.exp.cluster.genotype = AverageExpression(AddMetaData(burtoni.snseq.combined.sct.reduce.avp.neurons.recluster,
                                                             metadata = paste(burtoni.snseq.combined.sct.reduce.avp.neurons.recluster@meta.data$Genotype.id,
                                                                              burtoni.snseq.combined.sct.reduce.avp.neurons.recluster@meta.data$avp_clusters,
                                                                              sep = '_'),
                                                             col.name = 'Genotype.cluster'),
                                                 assays = 'SCT',
                                                 features = c('avp'),
...
avp.avg.exp.cluster.genotype.all = avp.avg.exp.cluster.genotype %>% 
  ...
  dplyr::select(Cluster,
                Avg.avp.expression,
                Genotype.id) %>% 
  dplyr::rename(avp_clusters = Cluster)
...
#### avp analysis #### 
## compare expression across social status
# fit gamma glm
glm_model = glm(Avg.expression  ~ orig.ident * Cluster, 
                 data = avp.avg.exp.cluster.genotype %>% 
                   mutate(orig.ident = as.factor(orig.ident),
                          Cluster = as.factor(Cluster),
                          Genotype.id = as.factor(Genotype.id)), 
                 family = Gamma(link = "log"))

# Run type III Analysis of Deviance
car::Anova(glm_model, 
           type = "III") 

# posthoc test
glm_means = emmeans::emmeans(glm_model,
                             ~ orig.ident * Cluster, 
                             type = "response")

# Compare status for each cluster
pairwise_by_cluster <- pairs(glm_means,
                             by = "Cluster",
                             adjust = "fdr")
summary(rbind(pairwise_by_cluster),
        adjust = 'fdr')

# Compare clusters within each status
pairwise_by_treatment <- pairs(glm_means, 
                               by = "orig.ident", 
                               adjust = "tukey")
summary(pairwise_by_treatment)

## compare expression across  clusters
# fit gamma glm
glm_mode.cluster = glm(Avg.expression  ~ Cluster, 
                data = avp.avg.exp.cluster.genotype %>% 
                  mutate(orig.ident = as.factor(orig.ident),
                         Cluster = as.factor(Cluster),
                         Genotype.id = as.factor(Genotype.id)), 
...
# Run type III Analysis of Deviance
car::Anova(glm_mode.cluster, 
           type = "III") 

# posthoc test
glm_means.cluster = emmeans::emmeans(glm_mode.cluster,
                             ~  Cluster, 
                             type = "response")

# Compare clusters 
pairwise_by_glm_means.cluster <- pairs(glm_means.cluster, 
                               adjust = "tukey")
summary(pairwise_by_glm_means.cluster)


## compare to neuron clusters
burtoni.snseq.combined.sct.all.avp.neurons.recluster@meta.data %>% 
  count(neuron_clusters, 
        avp_clusters) %>% 
  group_by(avp_clusters) %>% 
  mutate(total = sum(n),
         percent = 100*(n/total)) %>% 
  ungroup() %>% 
  filter(n>10) %>% 
  filter(percent>25) %>%
  ggplot(aes(y = neuron_clusters,
             x =avp_clusters, 
             fill = percent, 
             label = n)) +
  geom_tile() +
  ...
  theme_classic() +
  scale_fill_gradient2(low = 'grey100',
                      mid = 'grey50',
                       high = 'grey0',
                      midpoint = 50
                      ) +
  scale_alpha_continuous(range = c(0.5,
                                1))
ggsave('neuropeptides/avp.oxt/avp/Avp.clusters.vs.neuron.clusters.png',
       width = 10,
       height = 10)
...
ggsave('neuropeptides/avp.oxt/avp/Avp.clusters.vs.neuron.clusters.paper.pdf',
       width = 3.3,
       height = 3.3,
       units = 'in',
       dpi = 720)
...
#   ungroup() %>% 
#   filter(percent>5) %>%
# ggplot(aes(axis2 = neuron_clusters,
#            axis1 = avp_clusters,
#            y = n)) +
#   geom_stratum(width = 1/3) +
#   geom_alluvium(aes(fill = avp_clusters),
#                 width = 1/3) +
# 
#   geom_text(stat = "stratum",
#             aes(label = after_stat(stratum)), 
#             color = "black") +
#   scale_x_discrete(limits = c("avp_clusters",
#                               "neuron_clusters"
#                               ), 
#                    expand = c(0.15, 
#                               0.05)) +
...
burtoni.snseq.combined.sct.neurons.expression %>% 
  ...
  geom_point(data = burtoni.snseq.combined.sct.neurons.expression %>% 
               filter(!is.na(avp_clusters)),
             aes(color = avp_clusters)) +
  theme_classic()
...
burtoni.snseq.combined.sct.reduce.avp.neurons.recluster@meta.data %>% 
  droplevels() %>% 
  dplyr::select(c(avp_clusters,
           neuron_clusters)) %>% 
  table() %>% 
  as.data.frame() %>% 
  ggplot(aes(axis1 = neuron_clusters,
             axis2 = avp_clusters,
             y = Freq)) +
  geom_alluvium(aes(fill = avp_clusters)) +
  geom_stratum() +
  geom_text(stat = "stratum",
            aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("neuron_clusters", "avp_clusters"),
                   expand = c(0.15, 0.05)) +
...
#### IEG analysis ####
#get average expression data for each IEG
IEG.avp.avg.exp.cluster.genotype = AverageExpression(burtoni.snseq.combined.sct.reduce.avp.neurons.recluster,
                  assays = 'SCT',
                  features = c(IEG.list),
                  return.seurat = FALSE,
                  ...
                  verbose = TRUE) %>% 
  as.data.frame() %>% 
  rownames_to_column('IEG') %>% 
  pivot_longer(-c('IEG'),
               names_to = 'sample',
               values_to = 'avg_IEG_expression') %>% 
  separate_wider_delim(delim = '_',
                       cols = 'sample',
                       names = c('Genotype.id',
                                 'avp_clusters')) %>% 
  mutate(Genotype.id = str_sub(Genotype.id,
                               start = 5)) %>% 
...
#for poster
##graph IEG score per cluster across social status
IEG.avp.avg.exp.cluster.genotype %>%
  ggplot(aes(x = avp_clusters,
             y = avg_IEG_expression,
             color = orig.ident,
             group = orig.ident)) +
  ...
  facet_wrap(IEG~.,
             scales = 'free')
ggsave('neuropeptides/avp.oxt/avp/Average IEG expression per cluster and genotype poster update.pdf',
       height = 6.5,
       width = 6.5,
       units = "in",
       dpi = 720)
...
DotPlot(burtoni.snseq.combined.sct.reduce.avp.neurons.recluster,
  ...
  scale_y_discrete(labels = c('Dom 0',
                              ...
                              'Dom 6',
                              'Sub 6')) +
  scale_x_discrete(labels = c('IEG_score',
                              IEG.list)) +
  ...
  geom_hline(yintercept = 12.5,
             linetype = 'dashed')
ggsave('./neuropeptides/avp.oxt/avp/Dotplot avp IEG cluster status paper.pdf',
       height = 6.5,
       width = 6.5)

#### DEG analysis ####
### use run_de to perf
obj <- burtoni.snseq.combined.sct.reduce.avp.neurons.recluster
DefaultAssay(obj) <- "RNA"

obj$cell_type <- obj$avp_clusters
obj$replicate <- obj$Genotype.id
obj$label <- factor(obj$orig.ident, levels = c("sub_burtoni_snseq", "dom_burtoni_snseq"))
...
library(broom) # For cleaning up test results
...
df_long <- as.data.frame(as.matrix(detected)) %>%
...
df <- df_long %>%
  ...
  group_by(gene, avp_clusters) %>%
  filter(
    max(pct) >= 5 &          # detected in at least 5% of cells somewhere
      n_distinct(pct) > 1      # variation across replicates
  ) %>%
  ungroup()


# filter down to DEG and IEGlist
# 
avp.deg.list = DE_results %>% 
  filter(p_val_adj < 0.15) %>% 
  pull(gene) %>% 
  unique()

# remove mito genes
avp.deg.list = union(IEG.list, avp.deg.list)

avp.deg.list = avp.deg.list[!avp.deg.list %in% mito.genes.list]

df <- df %>% filter(gene %in% avp.deg.list)


# 1. Calculate the average percentage across the 3 genotypes for each condition
summary_stats <- df %>%
  group_by(gene, 
           avp_clusters, 
           orig.ident) %>%
  summarize(mean_pct = mean(pct), .groups = 'drop') %>%
  pivot_wider(names_from = orig.ident, values_from = mean_pct, names_prefix = "mean_pct_")


de_proportions <- df %>%
  group_by(gene, avp_clusters) %>%
  filter(n() == 6) %>%                     # all replicates present
  filter(sd(pct) > 0) %>%                  # <-- key fix
  do(tidy(t.test(pct ~ orig.ident, data = .))) %>%
  ungroup() %>%
  group_by(avp_clusters) %>% 
  mutate(fdr = p.adjust(p.value, method = "fdr")) %>% 
  ungroup()  %>% 
inner_join(summary_stats, by = c("gene", 
                                 "avp_clusters")) %>%
  mutate(pct_diff = mean_pct_dom_burtoni_snseq - mean_pct_sub_burtoni_snseq,
...
avp.neuron.reduce.DomvsSub.vector.list.cluster.sct.prep = burtoni.snseq.combined.sct.reduce.avp.neurons.recluster.expression.sct.prep %>% 
  mutate(avp.neuron.cluster = avp_clusters %>% 
           as.factor()) %>% 
...
run_limmatrend_complex_genotype.sct <- function(L) {
  ...
  timing <- system.time({
    ...
    contrasts <- makeContrasts(DvsS_0 = (treatDom.A.0 + treatDom.B.0 + treatDom.C.0)/3 - (treatSub.A.0 + treatSub.B.0 + treatSub.C.0)/3, 
                               ...
                               DvsS_5 = (treatDom.A.4 + treatDom.B.4 + treatDom.C.4)/3 - (treatSub.A.4 + treatSub.B.4 + treatSub.C.4)/3,
                               DvsS_6 = (treatDom.A.4 + treatDom.B.4 + treatDom.C.4)/3 - (treatSub.A.4 + treatSub.B.4 + treatSub.C.4)/3,
                               DvsS=(treatDom.A.0 + treatDom.B.0 + treatDom.C.0+treatDom.A.1 + treatDom.B.1 + treatDom.C.1+treatDom.A.2 + treatDom.B.2 + treatDom.C.2+treatDom.A.3 + treatDom.B.3 + treatDom.C.3+treatDom.A.4 + treatDom.B.4 + treatDom.C.4+treatDom.A.5 + treatDom.B.5 + treatDom.C.5 +treatDom.A.6 + treatDom.B.6 + treatDom.C.6)/21-(treatSub.A.0 + treatSub.B.0 + treatSub.C.0+treatSub.A.1 + treatSub.B.1 + treatSub.C.1+treatSub.A.2 + treatSub.B.2 + treatSub.C.2+treatSub.A.3 + treatSub.B.3 + treatSub.C.3+treatSub.A.4 + treatSub.B.4 + treatSub.C.4+treatSub.A.5 + treatSub.B.5 + treatSub.C.5+treatSub.A.6 + treatSub.B.6 + treatSub.C.6)/21,
                               levels = design)
    dge <- DGEList(L$count, 
                   group = treat)
  ...
  hist(tt5$P.Value, 50)
  hist(tt5$adj.P.Val, 50)
  hist(tt6$P.Value, 50)
  hist(tt6$adj.P.Val, 50)
...
avp.neuron.reduce.genotype.vector.limma.results.df.sct.treat.prep = full_join(avp.neuron.reduce.genotype.vector.limma.results.sct$tt0 %>% 
  ...
  full_join(avp.neuron.reduce.genotype.vector.limma.results.sct$tt4 %>% 
              ...
              rownames_to_column("Gene")) %>% 
  full_join(avp.neuron.reduce.genotype.vector.limma.results.sct$tt5 %>% 
              rename_with(~paste0(.,"_DvsS_5")) %>% 
              rownames_to_column("Gene")) %>% 
  full_join(avp.neuron.reduce.genotype.vector.limma.results.sct$tt6 %>% 
              rename_with(~paste0(.,"_DvsS_6")) %>% 
              rownames_to_column("Gene")) %>% 
...
avp.neuron.reduce.genotype.vector.limma.results.df.sct.treat.prep = avp.neuron.reduce.genotype.vector.limma.results.df.sct.treat.prep %>% 
  mutate(Sig_DvsS_0 = ifelse(adj.P.Val_DvsS_0 <= 0.1,
                             'Sig',
                             'Not Sig'),
         ...
         Sig.direction_DvsS_0 = ifelse(Sig_DvsS_0 == 'Sig',
                                       ...
                                       Sig_DvsS_0)) %>% 
  mutate(Sig_DvsS_1 = ifelse(adj.P.Val_DvsS_1 <= 0.1,
                             'Sig',
                             'Not Sig'),
         ...
         Sig.direction_DvsS_1 = ifelse(Sig_DvsS_1 == 'Sig',
                                       ...
                                       Sig_DvsS_1)) %>% 
  mutate(Sig_DvsS_2 = ifelse(adj.P.Val_DvsS_2 <= 0.1,
                             'Sig',
                             'Not Sig'),
         ...
         Sig.direction_DvsS_2 = ifelse(Sig_DvsS_2 == 'Sig',
                                       ...
                                       Sig_DvsS_2)) %>% 
  mutate(Sig_DvsS_3 = ifelse(adj.P.Val_DvsS_3 <= 0.1,
                             'Sig',
                             'Not Sig'),
         ...
         Sig.direction_DvsS_3 = ifelse(Sig_DvsS_3 == 'Sig',
                                       ...
                                       Sig_DvsS_3)) %>% 
  mutate(Sig_DvsS_4 = ifelse(adj.P.Val_DvsS_4 <= 0.1,
                             'Sig',
                             'Not Sig'),
         ...
         Sig.direction_DvsS_4 = ifelse(Sig_DvsS_4 == 'Sig',
                                       ...
                                       Sig_DvsS_4)) %>% 
  mutate(Sig_DvsS_5 = ifelse(adj.P.Val_DvsS_5 <= 0.1,
                             'Sig',
                             'Not Sig'),
         Direction.type_DvsS_5 = ifelse(logFC_DvsS_5 > 0,
                                        'up',
                                        'down'),
         Sig.direction_DvsS_5 = ifelse(Sig_DvsS_5 == 'Sig',
                                       Direction.type_DvsS_5,
                                       Sig_DvsS_5)) %>% 
  mutate(Sig_DvsS_6 = ifelse(adj.P.Val_DvsS_6 <= 0.1,
                             'Sig',
                             'Not Sig'),
         Direction.type_DvsS_6 = ifelse(logFC_DvsS_6 > 0,
                                        'up',
                                        'down'),
         Sig.direction_DvsS_6 = ifelse(Sig_DvsS_6 == 'Sig',
                                       Direction.type_DvsS_6,
                                       Sig_DvsS_6)) %>% 
...
avp.neuron.reduce.genotype.vector.limma.results.df.sct.treat.prep  %>%
  mutate(Keep = case_when(Sig_DvsS_0 != "Not Sig" ~ "Keep",
                          ...
                          Sig_DvsS_5 != "Not Sig" ~ "Keep",
                          Sig_DvsS_6 != "Not Sig" ~ "Keep",
  ...
  mutate(All = case_when(Direction.type_DvsS == 'down' ~ log10(adj.P.Val_DvsS),
         ...
         '4' = case_when(Direction.type_DvsS_4 == 'down' ~ log10(adj.P.Val_DvsS_4),
                         ...
                         TRUE ~ 0),
         '5' = case_when(Direction.type_DvsS_3 == 'down' ~ log10(adj.P.Val_DvsS_3),
                         Direction.type_DvsS_3 == 'up' ~ -log10(adj.P.Val_DvsS_3),
                         TRUE ~ 0),
         '6' = case_when(Direction.type_DvsS_3 == 'down' ~ log10(adj.P.Val_DvsS_3),
                         Direction.type_DvsS_3 == 'up' ~ -log10(adj.P.Val_DvsS_3),
                         TRUE ~ 0)) %>%
  select(c(All,
           '0',
...
# #### check gene dotplot in avp neurons ####
# ## dotplot
# # KCNMB4
# DotPlot(burtoni.snseq.combined.sct.reduce.avp.neurons.recluster,
#         features = c('KCNMB4'
...
burtoni.snseq.combined.sct.neurons.expression.avp.oxt.avg %>% 
  ...
  geom_point(data = burtoni.snseq.combined.sct.neurons.expression.avp.oxt.avg %>% 
               filter(AVP.oxt.cell == 'Avp' | AVP.oxt.cell == 'Both') %>% 
               arrange(desc(Avg.avp.expression.scale)),
             aes(color = Avg.avp.expression.scale,
                 size = Avg.avp.expression.scale)) +
  scale_colour_gradientn(colours = c("lightpink", "red4"),
                         limits = c(0,100)
  ...
  geom_point(data = burtoni.snseq.combined.sct.neurons.expression.avp.oxt.avg %>% 
               filter(AVP.oxt.cell == 'Oxt'| AVP.oxt.cell == 'Both')%>% 
               arrange(desc(Avg.oxt.expression.scale)),
             aes(color = Avg.oxt.expression.scale,
                 size = Avg.oxt.expression.scale,
                 alpha = Avg.oxt.expression.scale)) +
  scale_colour_gradientn(colours = c("dodgerblue", "blue4"),
...
## AVP cluster 5
avp.cluster5.markers.df <- FindMarkers(burtoni.snseq.combined.sct.neurons, 
                                       ident.1 = 11, 
                                       min.pct = 0.25, 
                                       assay = 'SCT')
#create gene column
avp.cluster5.markers.df = avp.cluster5.markers.df %>% 
  rownames_to_column('gene')
# create specificity score
avp.cluster5.markers.df = avp.cluster5.markers.df %>% 
  mutate(specificity = avg_log2FC*(pct.1/pct.2))
...
# add cluster id
avp.cluster5.markers.df = avp.cluster5.markers.df %>% 
  mutate(Avp.cluster.id = "5")

## AVP cluster 6
avp.cluster6.markers.df <- FindMarkers(burtoni.snseq.combined.sct.neurons, 
                                       ident.1 = 4, 
                                       min.pct = 0.25, 
                                       assay = 'SCT')
#create gene column
avp.cluster6.markers.df = avp.cluster6.markers.df %>% 
  rownames_to_column('gene')
# create specificity score
avp.cluster6.markers.df = avp.cluster6.markers.df %>% 
  mutate(specificity = avg_log2FC*(pct.1/pct.2))
...
avp.cluster.markers.df = avp.cluster0.markers.df %>% 
  ...
  rbind(avp.cluster5.markers.df) %>% 
  rbind(avp.cluster6.markers.df) 
