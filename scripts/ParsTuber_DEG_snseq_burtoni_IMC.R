...
#### clustering for parstuber ####
burtoni.snseq.combined.sct.parstuber = burtoni.snseq.combined.sct

#set idents
Idents(object = burtoni.snseq.combined.sct.parstuber) <- "sctypemarkers.hypo"

# check celltype names
burtoni.snseq.combined.sct.parstuber@meta.data$sctypemarkers.hypo %>% 
  unique()

#subset to parstuber
burtoni.snseq.combined.sct.parstuber = subset(burtoni.snseq.combined.sct.parstuber,
                                            idents = c("C7-6: ParsTuber"))

#remove large file
# rm(burtoni.snseq.combined.sct)

# need to set to integrated for clustering
DefaultAssay(burtoni.snseq.combined.sct.parstuber) = 'integrated'

#check data loaded correctly
## run PCA, UMAP, and cluster 
#use 0.2 resolution
burtoni.snseq.combined.sct.parstuber = burtoni.snseq.combined.sct.parstuber %>% 
  RunPCA() %>%
...
Idents(object = burtoni.snseq.combined.sct.parstuber) <- "integrated_snn_res.0.2"

DimPlot(burtoni.snseq.combined.sct.parstuber,
        group.by='integrated_snn_res.0.2',
        label=TRUE) +
  ggtitle('parstuber') +
  NoLegend() +
  theme_classic()
ggsave('parstuber/UMAP parstuber clusters.png',
       height = 5.5,
       width = 5.5)
...
DimPlot(burtoni.snseq.combined.sct.parstuber,
  ...
  ggtitle('parstuber')+
  theme_classic()
ggsave('parstuber/UMAP parstuber genotypes.png',
       height = 5.5,
       width = 5.5)

## create count across genotype per cluster
burtoni.snseq.combined.sct.parstuber@meta.data %>%
  as.data.frame() %>%
  ...
  geom_jitter(height = 0,
              width = 0.1) +
  theme_classic()
ggsave('parstuber/Genotype per clusters count.png',
       height = 5.5,
       width = 5.5)
...
# add parstuber metadata
UMAP.orig = UMAP.orig %>% 
  full_join(burtoni.snseq.combined.sct.parstuber@meta.data %>%
                        as.data.frame() %>% 
                        rownames_to_column('Cell.id') %>% 
                        select(c(Cell.id,
                                 integrated_snn_res.0.2))) 
...
UMAP.orig %>% 
  ...
  theme_classic() +
  labs(color = 'parstuber clusters')
ggsave('parstuber/UMAP projection parstuber clusters.png',
       height = 10,
       width = 12.5)
...
burtoni.snseq.combined.sct.parstuber.clustree <- Seurat::FindClusters(object = burtoni.snseq.combined.sct.parstuber,
                                                                               resolution = resolution.range.reduced.2)
#check data
# head(burtoni.snseq.combined.sct.parstuber.recluster.clustree[[]])
...
#                         '#b3b3b3')
#clustree
clustree(burtoni.snseq.combined.sct.parstuber.clustree,
...
ggsave('parstuber/Clustree parstuber.png',
       height = 10,
       width = 10)

# remove large file
rm(burtoni.snseq.combined.sct.parstuber.clustree)
...
burtoni.snseq.combined.sct.parstuber.variable.prep <- FindVariableFeatures(burtoni.snseq.combined.sct.parstuber, 
                                                                                              assay = 'integrated',
                                                                                              selection.method = "vst", 
                                                                                              nfeatures = 1500, 
                                                                                              verbose = F)
# identify top 1500 variable genes
parstuber.topgenes.prep <- head(VariableFeatures(burtoni.snseq.combined.sct.parstuber.variable.prep,
                                                               assay = 'integrated'), 
                                              1500)

# create dummy
burtoni.snseq.combined.sct.parstuber.expression.sct.prep = full_join(full_join(burtoni.snseq.combined.sct.parstuber@reductions$umap@cell.embeddings %>% 
                                                                                                   as.data.frame() %>% 
                                                                                                   rownames_to_column("Cell.id"),
                                                                                burtoni.snseq.combined.sct.parstuber@meta.data %>%
                                                                                                   rownames_to_column("Cell.id")),
                                                                      burtoni.snseq.combined.sct.parstuber@assays$SCT@data %>% 
                                                                                         as.data.frame()  %>% 
                                                                                         t() %>% as.data.frame() %>% 
                                                                                         rownames_to_column('Cell.id'))

## create vector of factor
parstuber.DomvsSub.vector.list.sct.prep = burtoni.snseq.combined.sct.parstuber.expression.sct.prep %>% 
  mutate(parstuber.orig.ident = orig.ident %>% 
           as.factor()) %>% 
  pull(parstuber.orig.ident) %>% 
  droplevels()
...
parstuber.DomvsSub.vector.count.sct.prep = GetAssayData(burtoni.snseq.combined.sct.parstuber,
                                                                assay = 'SCT') %>% 
  as_tibble(rownames = NA) %>% 
  rownames_to_column('gene') %>% 
  dplyr::select(c(gene,
                  burtoni.snseq.combined.sct.parstuber.expression.sct.prep %>% 
                    pull(Cell.id))) %>% 
  filter(gene %in% parstuber.topgenes.prep) %>% 
  column_to_rownames('gene') %>% 
  as.matrix() %>% 
  pmax(0)


## run with parstuber clusters
#need list with L with  count and condt
# add in cell cluster with condt.2
## create vector of factor
parstuber.DomvsSub.vector.list.cluster.sct.prep = burtoni.snseq.combined.sct.parstuber.expression.sct.prep %>% 
  mutate(parstuber.cluster = integrated_snn_res.0.2 %>% 
           as.factor()) %>% 
  pull(parstuber.cluster) %>% 
  droplevels


# add in genotype
## create vector of factor
parstuber.DomvsSub.vector.list.genotype.sct.prep = burtoni.snseq.combined.sct.parstuber.expression.sct.prep %>% 
...
#create list
parstuber.vector.limma.sct.prep = list(count = parstuber.DomvsSub.vector.count.sct.prep,
                                               condt.2 = parstuber.DomvsSub.vector.list.cluster.sct.prep,
                                               genotype = parstuber.DomvsSub.vector.list.genotype.sct.prep)
# add genotype.ID?
...
run_limmatrend_complex_genotype.sct <- function(L) {
  ...
  timing <- system.time({
    ...
    contrasts <- makeContrasts(DvsS_0 = (treatDom.A.0 + treatDom.B.0 + treatDom.C.0)/3 - (treatSub.A.0 + treatSub.B.0 + treatSub.C.0)/3, 
                               ...
                               DvsS=(treatDom.A.0 + treatDom.B.0 + treatDom.C.0+treatDom.A.1 + treatDom.B.1 + treatDom.C.1)/6-(treatSub.A.0 + treatSub.B.0 + treatSub.C.0+treatSub.A.1 + treatSub.B.1 + treatSub.C.1)/6,
                               levels = design)
    dge <- DGEList(L$count, 
                   group = treat)
  ...
  # # Open pdf file
  # pdf(file= "./parstuber/limmatrend/limmatrend.histograms.pdf" )
  # # create a 2X2 grid
  # par( mfrow= c(2,2) )
  # #graph
  # hist(tt0$P.Value, 50)
  ...
  # 
  # # Open pdf file
  # pdf(file= "./parstuber/limmatrend/limmatrend.MDS.pdf" )
  # # create a 2X1 grid
  # par( mfrow= c(2,1) )
  # limma::plotMDS(dge, 
...
###run function  
parstuber.genotype.vector.limma.results.sct = run_limmatrend_complex_genotype.sct(parstuber.vector.limma.sct.prep)

# save results to dataframe
parstuber.genotype.vector.limma.results.df.sct.treat.prep = full_join(parstuber.genotype.vector.limma.results.sct$tt0 %>% 
                                                                                rename_with(~paste0(.,"_DvsS_0")) %>% 
                                                                                rownames_to_column("Gene"),
                                                                              parstuber.genotype.vector.limma.results.sct$tt1 %>% 
                                                                                rename_with(~paste0(.,"_DvsS_1")) %>% 
                                                                                rownames_to_column("Gene")) %>% 
  full_join(parstuber.genotype.vector.limma.results.sct$ttDS %>% 
...
parstuber.genotype.vector.limma.results.df.sct.treat.prep = parstuber.genotype.vector.limma.results.df.sct.treat.prep %>% 
...
for (i in limma.genotpye.vector) {
  # graph volcano plot
  parstuber.genotype.vector.limma.results.df.sct.treat.prep %>% 
  ...
  ggsave(paste0('parstuber/limmatrend/limma.parstuber.volcano.', i, '.png'),
         width = 5,
         height = 5)
}
...
#### save results ####
# save to csv
write_csv(parstuber.genotype.vector.limma.results.df.sct.treat.prep,
          file = 'parstuber/limmatrend/parstuber.limmatrend.results.csv')

##### poster heatmap ####
library(pheatmap)

# create matrix
parstuber.deg.matrix = parstuber.genotype.vector.limma.results.df.sct.treat.prep %>%
  filter(Keep == "Keep") %>% 
  column_to_rownames('Gene') %>% 
  select(c(All,
           '0',
...
parstuber.deg.matrix[!is.finite(parstuber.deg.matrix)] <- parstuber.deg.matrix %>% 
  as.data.frame() %>% 
...
parstuber.deg.matrix %>% 
  pheatmap(cluster_rows = F,
           ...
           filename = "parstuber/limmatrend/Heatmap DEGs across clusters.pdf",
           width = 12,
           height = 5
  )
#png
parstuber.deg.matrix %>% 
  pheatmap(cluster_rows = F,
           ...
           fontsize = 15,
           breaks = seq(-4, 4, length.out = 100),
           filename = "parstuber/limmatrend/Heatmap DEGs across clusters.png",
           width = 12,
           height = 5
  )

# create heatmap
parstuber.deg.phtmap <- pheatmap::pheatmap(parstuber.deg.matrix,
                                            cluster_rows = F,
                                            cluster_cols = T,
                                            scale = 'none',
                                            silent = T)
...
# get order list
parstuber.deg.order.list = c(parstuber.deg.phtmap$tree_col$order)
# create label list
parstuber.deg.order = data.frame(label = parstuber.deg.phtmap$tree_col$labels) %>% 
  rownames_to_column('position')
# combine labels and position
parstuber.deg.order = parstuber.deg.order[match(parstuber.deg.order.list,
                                                  parstuber.deg.order$position),] %>% 
  mutate(parstuber.deg.order.list = 1:length(parstuber.deg.order.list)) %>% 
  mutate(position = as.numeric(position))
# create new order list
parstuber.deg.order.list.new = parstuber.deg.order %>%
  arrange(position) %>% 
  pull(label)

# move Dom bias genes to end
parstuber.deg.col_dend <- parstuber.deg.phtmap[[2]]
parstuber.deg.col_dend <- dendextend::rotate(parstuber.deg.col_dend, 
                                              order = parstuber.deg.order.list.new)
# graph pheat map
# pdf
pheatmap(parstuber.deg.matrix, 
         cluster_cols=as.hclust(parstuber.deg.col_dend),
         cluster_rows = F,
         gaps_row = 1,
         scale = 'none',
         border_color = 'black',
         ...
         filename = "parstuber/limmatrend/Heatmap DEGs across clusters reorder.pdf",
         width = 12,
         height = 5
)
...
pheatmap(parstuber.deg.matrix, 
         ...
         fontsize = 15,
         breaks = seq(-2, 2, length.out = 100),
         filename = "parstuber/limmatrend/Heatmap DEGs across clusters reorder.png",
         width = 12,
         height = 5
)