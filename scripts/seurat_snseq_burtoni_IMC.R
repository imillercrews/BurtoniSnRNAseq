...
# # check gene names and ensemble ID
# tmp.gene.list = read.table("/stor/work/Hofmann/All_projects/A_burtoni_snseq/cellranger/count/NileTilapia.reference/dom/Burtoni-dom/outs/filtered_feature_bc_matrix/features.tsv.gz")
...
# get percent mito genes
dom.object[["percent.mt"]] <- PercentageFeatureSet(dom.object, 
                                            features = intersect(mito.genes.list,
                                                                 rownames(dom.object)),
                                            assay = "RNA")
...
## QC graph
# violin plot
FetchData(object = dom.object, 
            vars = c("nFeature_RNA", 
                     "nCount_RNA",
                     "percent.mt",
                     "orig.ident")) %>% 
  dplyr::rename(`Detected Genes` = nFeature_RNA, 
         `Total Molecules` = nCount_RNA, 
  ...
  ggplot(aes(x = Type,
    ...
    geom_jitter(size = 0.5,
                alpha = 0.2, 
                width = 0.2) + 
  geom_hline(data = data.frame(Type = c("Detected Genes", 
                                        "Detected Genes",
                                        "Mitochondrial %",
                                        "Total Molecules"),
                               threshold = c(2000, 
...
#filter
FeatureScatter(subset(dom.object, 
                      subset = c(nCount_RNA < 6000 &
                                 nFeature_RNA < 2000 &
                                   nFeature_RNA > 1000 &
                                   percent.mt < 5)),
               feature1 = "percent.mt",
               feature2 = "nFeature_RNA")
ggsave('dom/featurescatter.dom.limits.filter.png')

#histogram
dom.object@meta.data %>% 
  subset(nCount_RNA < 6000 & nFeature_RNA < 3000 & percent.mt < 5) %>%
...
#count 14534 nuclei 
dom.object@meta.data %>% 
  subset(nCount_RNA < 6000 &  nFeature_RNA < 2000 & nFeature_RNA > 1000 & percent.mt < 5) %>%
  summarize(max = max (nFeature_RNA),
            min = min (nFeature_RNA),
            count = n())

## filter data
dom.object.SCTransform <- subset(dom.object, 
                                 subset = nCount_RNA < 6000 &
                                   nFeature_RNA < 2000 &
                                   nFeature_RNA > 1000 &
                                   percent.mt < 5)
...
dom.object.SCTransform@meta.data %>% 
  summarize(max = max (nFeature_RNA),
            min = min (nFeature_RNA),
            count = n(),
            median.nCount = median(nCount_RNA),
            median.nFeature = median(nFeature_RNA),)

## run SCTransform, PCA, UMAP, and cluster 
#use 0.8 resolution from clustree
# uses 2000 genes and 5000 cells
...
## label mitochondria genes
# get percent mito genes
sub.object[["percent.mt"]] <- PercentageFeatureSet(sub.object, 
                                                   features = intersect(mito.genes.list,
                                                                        rownames(sub.object)),
                                                   assay = "RNA")
...
FeatureScatter(subset(sub.object, 
               ...
               feature1 = "percent.mt",
               feature2 = "nFeature_RNA")
ggsave('sub/featurescatter.sub.limits.filter.png')

#histogram
sub.object@meta.data %>% 
  subset(nFeature_RNA < 3000 & percent.mt < 5) %>%
  mutate(Kept = ifelse(nFeature_RNA > 1000 & nFeature_RNA < 2000,
                       'Kept',
                       'Remove')) %>% 
  ggplot(aes(nFeature_RNA,
...
## filter data
sub.object.SCTransform <- subset(sub.object, 
                                 subset = nCount_RNA < 6000 &
                                   nFeature_RNA < 2000 &
                                   nFeature_RNA > 1000 &
                                   percent.mt < 5)
...
# remove objects
rm(dom.object.SCTransform.filter.2)
rm(dom.object.SCTransform.filter.clustree)
rm(dom.object.SCTransform.filter.clustree.reduced)
...
# remove objects
rm(sub.object.SCTransform.filter.2)
rm(sub.object.SCTransform.filter.clustree)
rm(sub.object.SCTransform.filter.clustree.reduced)
...
# remove 
rm(burtoni.snseq.combined.sct.clustree)
...
# median UMI, sum UMI, median genes, sum genes, cell counts
burtoni.snseq.combined.sct@meta.data %>% 
  group_by(orig.ident) %>% 
  summarise(nFeature_RNA_sum = sum(nFeature_RNA),
            nFeature_RNA_median = median(nFeature_RNA),
            nCount_RNA_sum = sum(nCount_RNA),
            nCount_RNA_median = median(nCount_RNA),
            Cell_count = n()) %>% 
  View()

#### markers per clusters ####
## prepare for find all markers
...
DoHeatmap(burtoni.snseq.combined.sct,
  ...
  NoLegend()
ggsave('comparison/Top 3 marker genes per cluster.png')

# dotplot
DotPlot(burtoni.snseq.combined.sct,
          features = burtoni.snseq.combined.sct.markers.top10 %>% 
            pull(gene)) +
  coord_flip() +
  theme()
ggsave('comparison/Top 3 marker genes per cluster dotplot.png',
       height = 20,
       width = 15)

# save marker genes
write_tsv(burtoni.snseq.combined.sct.markers, 
          'burtoni.snseq.combined.sct.markers.tsv')
...
## save out barcodes
burtoni.snseq.selected.barcodes = burtoni.snseq.combined.sct@meta.data %>%
  rownames_to_column('Barcode.id') %>% 
...
#### trash ####
# # Identify CRH+ neurons
# crh_cells <- WhichCells(seurat_obj, expression = crh > 0)
# crh_subset <- subset(seurat_obj, cells = crh_cells)
# 
# # Normalize and scale (optional if already normalized)
# crh_subset <- NormalizeData(crh_subset)
# crh_subset <- FindVariableFeatures(crh_subset)
# crh_subset <- ScaleData(crh_subset)
# 
# # Differential gene expression: Dom vs Sub within CRH neurons
# markers_dom_vs_sub_crh <- FindMarkers(
#   crh_subset,
#   ident.1 = "Dom",
#   ident.2 = "Sub",
#   group.by = "Status",
...
# 
# # output
# head(markers_dom_vs_sub_crh)
# write.csv(markers_dom_vs_sub_crh, "crh_dom_vs_sub_DEGs.csv")
# 
# 
# # Identify CRH+ neurons
# crh_cells <- WhichCells(dom.object, expression = crhb > 0)
# crh_subset <- subset(seurat_obj, cells = crh_cells)
# 
# # Normalize and scale (optional if already normalized)
# crh_subset <- NormalizeData(crh_subset)
