...
### assign cell types to clusters
# get cell-type by cell matrix
es.max = sctype_score(scRNAseqData = burtoni.snseq.combined.sct[["integrated"]]@scale.data,
                      scaled = TRUE,
                      gs = gs_list$gs_positive,
                      gs2 = gs_list$gs_negative,
                      gene_names_to_uppercase = FALSE)
...
## use graph: './sctype.hypo/sctype score percentage per cluster.png'
...
data.cluster.sctype.hypo = data.cluster.sctype.hypo %>% 
  mutate(sctypemarkers.hypo.broad = case_when(seurat_clusters == 2 ~ " GABA",
                                              seurat_clusters == 6 ~ " GLU",
                                              seurat_clusters == 7 ~ " GLU",
                                              TRUE ~ sctypemarkers.hypo.broad)) %>% 
  mutate(sctypemarkers.hypo = case_when(seurat_clusters == 2 ~ "C7-2: GABA",
                                        seurat_clusters == 6 ~ "C7-1: GLU",
                                        seurat_clusters == 7 ~ "C7-1: GLU",
...
burtoni.snseq.combined.sct@meta.data %>% 
  ...
  ylab("Cells assigned (%)") +
  xlab('') 
ggsave('sctype.hypo/unknown/Percent cell broad per status.png',
       height = 6,
       width = 6)
...
burtoni.snseq.combined.sct@meta.data %>% 
  ...
  ylab("Cells assigned (%)") +
  xlab('') 
ggsave('sctype.hypo/unknown/Percent cell broad per status.pdf',
       height = 6,
       width = 6.5,
       dpi = 720,
       units = 'in')
...
ggsave('sctype.hypo/UMAP cell broad paper labels.pdf',
       height = 7,
       width = 7,
       units = 'in',
       dpi = 720)
...
ggsave('sctype.hypo/UMAP cell broad paper clusters.pdf',
       height = 7,
       width = 7,
       units = 'in',
       dpi = 720)
...
DimPlot(burtoni.snseq.combined.sct,
  ...
  xlab('Dim 1') +
  ylab('Dim 2') 
ggsave('sctype.hypo/UMAP cell broad paper.pdf',
       height = 7,
       width = 7,
       units = 'in',
       dpi = 720)
...
ggtree::ggtree(tree, aes(x, y)) +
  ...
  ggtree::theme_tree() +
  ggtree::geom_tiplab(offset = 1) +
  ggtree::geom_tippoint(aes(color = sctypemarkers.hypo),
                        shape = 16,
                        size = 5) +
  coord_cartesian(clip = 'off') +
  theme(plot.margin = unit(c(0,2.5,0,0),
...
ggtree(tree, aes(x, y)) +
  ...
  theme(plot.margin = unit(c(0,0.5,0,0),
                           'in')) +
  scale_color_manual(values = c('C7-3: Astro-Ependymal'  ='#F8766D',
                                'C7-2: GABA'  ='#B79F00',
                                'C7-1: GLU'  ='#00BA38',
                              'C7-4: Oligo+Precursor'  ='#00BFC4',
                              'C7-6: ParsTuber'  = '#619CFF',
                              'C7-7: Vascular'  ='#F564E3',
                              'Unknown'  ='grey')) +
  theme(legend.position = 'none') 
ggsave('sctype.hypo/unknown/Cluster cell type tree paper.pdf',
       height = 6.5,
       width = 3,
       units = 'in',
       dpi = 720,
